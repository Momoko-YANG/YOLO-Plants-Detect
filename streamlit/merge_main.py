...
# ==================== 全局配置 ====================
# 避免端口冲突：Flask用5000，Streamlit用8501（默认）
FLASK_HOST = '0.0.0.0'
FLASK_PORT = 5000
STREAMLIT_PORT = 8501
...
class VideoProcessingApp:
    def __init__(self, host='0.0.0.0', port=5000):
        ...
        self.socketio = SocketIO(self.app, cors_allowed_origins="*", async_mode='threading')  # 关键：指定async_mode为threading
        self.host = host
        self.port = port
        self.setup_routes()
        self.data = {}  # 存储接收参数
    ...
    def run(self):
        ...
        if not stop_event.is_set():
            self.socketio.run(
                ...
                debug=False,  # 关闭debug避免重复启动
                use_reloader=False  # 关键：禁用重载器，否则多线程冲突
            )
    ...
    def suggest(self):
        try:
            ...
            prompt = f"""
                你是一个消防专家，请根据识别的结果（火焰或者烟雾）给出相应的建议
                识别结果：{result}。
                结构：1.引起原因；2.自救技巧；3.治理方式；
    ...
    def predictImg(self):
        ...
        # 这里假设predictImg模块正确实现（注意：需确保predict.py存在）
        try:
            ...
            self.data.update({"status": 500, "message": f"预测失败：{str(e)}"})
        
        # 清理临时文件
        path = self.data.get("inputImg", "").split('/')[-1]
            ...
            def generate():
                ...
                try:
                    ...
                    if class_counter:
                        ...
                        self.socketio.emit(
                            'most_frequent_class', 
                            {"data": {'name': most_frequent_name, 'count': most_frequent_count, 'desc': formatted_response}},
...
def run_streamlit():
    ...
    with st.sidebar:
        ...
        st.title("帮助")
        st.info("""
            这是一个基于LangChain + DeepSeek AI的智能聊天系统。  
            您可以:  
            - 输入问题获取AI回答  
            - 管理多个会话  
            - 对回答进行反馈  
    ...
    if prompt := st.chat_input("请输入您的问题..."):
        ...
        if 'retrieval_system' not in st.session_state:
            st.session_state.retrieval_system = QARetrievalSystem()
            st.session_state.retrieval_system.load_qa_pairs()
        
        # 检索相关知识
        results = st.session_state.retrieval_system.search(prompt, top_n=5, threshold=0.1)
...
def main():
    ...
    print("=== 启动服务 ===")
    print(f"Flask服务将运行在: http://{FLASK_HOST}:{FLASK_PORT}")
    print(f"Streamlit服务将运行在: http://localhost:{STREAMLIT_PORT}")
    print("按 Ctrl+C 停止所有服务")

    # 1. 启动Flask服务（子线程）
    flask_app_instance = VideoProcessingApp(host=FLASK_HOST, port=FLASK_PORT)
    flask_thread = threading.Thread(target=flask_app_instance.run, daemon=True)
    flask_thread.start()
    time.sleep(2)  # 等待Flask服务初始化完成

    # 2. 启动Streamlit服务（主线程）
    # 注意：Streamlit需要在主线程运行，因此通过命令行方式启动
    import subprocess
    import sys
    streamlit_cmd = [
        sys.executable, "-m", "streamlit", "run", 
        __file__, "--server.port", str(STREAMLIT_PORT),
        "--server.headless", "true",  # 无头模式（无浏览器自动打开）
        "--server.enableCORS", "false"
    ]
    subprocess.run(streamlit_cmd)

if __name__ == "__main__":
    # 区分：如果是Streamlit调用，则只运行Streamlit部分；否则运行主程序
    if 'streamlit' in sys.argv[0] or 'STREAMLIT_SERVER_PORT' in os.environ:
        run_streamlit()
    else:
        try:
            main()
            ...
            if flask_app_instance:
                flask_app_instance.socketio.stop()
            sys.exit(0)